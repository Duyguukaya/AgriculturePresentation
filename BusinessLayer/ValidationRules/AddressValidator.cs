...
namespace BusinessLayer.ValidationRules
{
    public class AddressValidator:AbstractValidator<Address>
    {
        ...
        public AddressValidator()
        {
            ...
            RuleFor(x => x.Description1).MaximumLength(50).WithMessage("Açıklamayı kısaltın");
            RuleFor(x => x.Description2).MaximumLength(50).WithMessage("Açıklamayı kısaltın");
            RuleFor(x => x.Description3).MaximumLength(50).WithMessage("Açıklamayı kısaltın");
            RuleFor(x => x.Description4).MaximumLength(50).WithMessage("Açıklamayı kısaltın");
        }
    }
}
